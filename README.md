...
2.  **Configure Terraform:**
    ...
    - Create a `terraform.tfvars` file by copying the example:
      ...
      ```
    - Edit `terraform.tfvars` to match your environment. You will need to provide your Proxmox API URL, API token, and other details.
...
3.  **Provision the infrastructure:**
    ...
    - Initialize Terraform:
      ...
      terraform init
      ```
    - Plan the deployment to preview the changes:
      ```bash
      terraform plan
      ```
    - Apply the changes to create the VMs:
      ```bash
      terraform apply
      ```
...
4.  **Configure Ansible:**
    ...
    - The Ansible inventory is automatically generated by Terraform at `ansible/inventory/inventory.auto.yml`.
...
5.  **Run the Ansible playbook:**
    ...
    This will configure the K3s cluster and deploy all the applications.

## System Architecture

The homelab is built on a foundation of Proxmox for virtualization. Terraform is used to provision the virtual machines for the K3s cluster, and Ansible is used for configuration management and application deployment.

```mermaid
graph TD
    A[Proxmox] --> B(Terraform);
    B --> C{K3s Cluster};
    C --> D(Ansible);
    D --> E[Core Services];
    D --> F[Applications];
...
| **Coder**         | A remote development environment.                                                                       |
...
| **Homepage**      | A simple, static homepage for your homelab.                                                             |
